...
/**
Manager de l ' entité MvtMajCapital
@author Nom Prenom (email) 
*/
public class MvtMajCapitalManager extends MouvementManager {
...
*/
	  protected void doValidateCreateEntity(IEntite entite) throws ValidateEntiteException, EntiteException
	  {
		...
		if (((MvtMajCapital)entite).getRefTypeMajCapital() == null){
		    	ValidateEntiteException ve = new ValidateEntiteException ("FIELD.OBLIGATOIRE");
				ve.setValues(new String []{"typeMajCapital"});
				throw ve;
...
*/
	protected IEntite habiller(IEntite entite) throws EntiteException {
		MvtMajCapital mvtMajCapital = (MvtMajCapital) super.habiller(entite);


		
		TypeMajCapital refTypeMajCapital1 = mvtMajCapital.getRefTypeMajCapital();
		TypeMajCapital refTypeMajCapital = new TypeMajCapital();

		
		if (refTypeMajCapital1 != null) {
			((IEntite)refTypeMajCapital).setId(refTypeMajCapital1.getId());
		}
		if(refTypeMajCapital!=null && refTypeMajCapital1 != null){
			if (((IEntite)refTypeMajCapital).getId()!=0){
				    refTypeMajCapital =(TypeMajCapital)(new TypeMajCapital()).getFactory().newEntiteManager().getEntite((TypeMajCapital)refTypeMajCapital);
				mvtMajCapital.setRefTypeMajCapital(refTypeMajCapital);
			}
			else {
				mvtMajCapital.setRefTypeMajCapital(refTypeMajCapital1);
			}
		}		
		return mvtMajCapital;	
	}
/**
...
protected IEntite doProcessModifyEntity(IEntite entite)
	...
	MvtMajCapital mvtMajCapital = (MvtMajCapital) entite;
