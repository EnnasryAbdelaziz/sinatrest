...
import eai.devass.gsr.appli.modele.metier.mouvements.MvtRcptCertif;
...
/**
Manager de l ' entité MvtRcptCertif
@author Nom Prenom (email) 
*/
public class MvtRcptCertifManager extends MouvementManager {
...
*/
	  protected void doValidateCreateEntity(IEntite entite) throws ValidateEntiteException, EntiteException
	  {
		...
		if (((MvtRcptCertif)entite).getRefTypeCertificat() == null){
		    	ValidateEntiteException ve = new ValidateEntiteException ("FIELD.OBLIGATOIRE");
				ve.setValues(new String []{"typeCertificat"});
				throw ve;
...
*/
	protected IEntite habiller(IEntite entite) throws EntiteException {
		MvtRcptCertif mvtRcptCertif = (MvtRcptCertif) super.habiller(entite);


		
		TypeCertificat refTypeCertificat1 = mvtRcptCertif.getRefTypeCertificat();
		TypeCertificat refTypeCertificat = new TypeCertificat();

		
		if (refTypeCertificat1 != null) {
			((IEntite)refTypeCertificat).setId(refTypeCertificat1.getId());
		}
		if(refTypeCertificat!=null && refTypeCertificat1 != null){
			if (((IEntite)refTypeCertificat).getId()!=0){
				    refTypeCertificat =(TypeCertificat)(new TypeCertificat()).getFactory().newEntiteManager().getEntite((TypeCertificat)refTypeCertificat);
				mvtRcptCertif.setRefTypeCertificat(refTypeCertificat);
			}
			else {
				mvtRcptCertif.setRefTypeCertificat(refTypeCertificat1);
			}
		}		
		return mvtRcptCertif;	
	}
/**
...
protected IEntite doProcessModifyEntity(IEntite entite)
	...
	MvtRcptCertif mvtRcptCertif = (MvtRcptCertif) entite;
