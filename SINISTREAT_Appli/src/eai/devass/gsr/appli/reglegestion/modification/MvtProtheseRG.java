...
public void regleGestion993AlimenterProthese(EntiteBO entiteBO, Map params) throws ExceptionMetier {
		...
		if(commonGsrUtils.isEmpty(listProtheses)) {
			return;
				...
				for (int i = 0; i < listQuittances.size(); i++) {
					QuittanceGsr quittanceGsr = listQuittances.get(i);
					for (int j = 0; j < listProtheses.size(); j++) {
						if(i==j){
							Prothese prothese = listProtheses.get(i);
							if(prothese.getRefEtatProthese().getId()==eai.devass.gsr.appli.prm.EtatProthese.Supprimee.getId()){
								
								EtatQtc etatQtc = new EtatQtc(EtatQuittance.Supprimee.getId());
								quittanceGsr.setRefEtatQtc(etatQtc);
								
							}
							prothese.setDateEtat(new GregorianCalendar());
							if(prothese.getRefEtatProthese().getId()==eai.devass.gsr.appli.prm.EtatProthese.Cree.getId()){
							EtatProthese etatProthese = new EtatProthese(eai.devass.gsr.appli.prm.EtatProthese.Modifiee.getId());
							prothese.setRefEtatProthese(etatProthese);
							}
							quittanceGsr.setRefMouvement(mvtProthese);
							prothese.setRefQuittanceGSR(quittanceGsr);
							prothese.setRefRentier(rentierDB);
							prothese.setContextRegleGestion(ContextRegleGestion.MODIFICATION.getContext());
