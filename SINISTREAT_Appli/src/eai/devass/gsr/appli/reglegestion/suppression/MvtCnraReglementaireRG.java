...
private Long idSinistre = null;
	...
	public void regleGestion000ListeRentier(EntiteBO entiteBO, Map params) throws ExceptionMetier {
		...
		MvtCnraReglementaire mvtCnraReglementaire = (MvtCnraReglementaire) entiteBO;
		
		// REcuperer le rentier
		MvtCnraReglementaire mvtConsignCNRADB = (MvtCnraReglementaire) getSession().get(
				MvtCnraReglementaire.class, mvtCnraReglementaire.getId());
	...
	public void regleGestion001ListeMvtConsignation(EntiteBO entiteBO, Map params) throws ExceptionMetier {
		...
		List<MvtCnraReglementaire> listMvtConsignCNRA = null;
			...
			for(Rentier curRentier : listRentierDB) {
				if (sinistreBD.getRefVictime().getDeces() != null 
					...
					if (sinistreBD.getRefVictime().getDeces() == false
						...
						traitementMvtCnra(entiteBO,curRentier,etatMvt);
	...
	private void traitementMvtCnra(EntiteBO entiteBO,
			Rentier curRentier, EtatMvt etatMvt) throws Exception {

		List<MvtCnraReglementaire> listMvtCnraReglementaire = null;
		MvtCnraReglementaire curMvtConsignCNRA = null;
		SituationMouvement situationMouvement = null;
		List<QuittanceGsr> listQuittance = null;
		SituationQuittanceGsr situationQuittanceGsr = null;
		// REcuperer le mouvement du rentier
		listMvtCnraReglementaire = rentierManager
				.getLastMvtCnraReglementaireByRentier(curRentier.getId(),
						etatMvt);
		...
		curMvtConsignCNRA = listMvtCnraReglementaire.get(0);
