package eai.devass.gsr.appli.usecase.metier.dossierRente;

import java.util.ArrayList;
import java.util.List;

import ma.co.omnidata.framework.services.businessInterface.ValidationException;
import ma.co.omnidata.framework.services.businessInterface.impl.RechListeVOConverter;
import eai.devass.gsr.appli.modele.metier.dossierRente.Certificats;
import eai.devass.gsr.appli.valueobjects.metier.dossierRente.CertificatsVO;
import eai.devass.gsr.appli.valueobjects.metier.dossierRente.CertificatsVOConverter;


/**
Converter du use case de recherche
@author Nom Prenom (email)
*/

public class CertificatsRechercherUCConverter extends RechListeVOConverter {


	CertificatsVOConverter certificatsVOConverter = new CertificatsVOConverter();
/**
Methode qui convertit une entité à un Value Object
@param item entité à convertir
@returns Value Object resultat  converti
*/	
	protected Object convertItemtoVOItem(Object item) {
		CertificatsVO vo = new CertificatsVO();
		certificatsVOConverter.itemToVo(vo, (Certificats)item, null);
		return vo;
	}
/**
Methode qui convertit un Value Object à une liste d' entités
@param ovo value object à convertir
@returns Listes des entités resultats convertis
@throws ValidationException Probleme lors de la validation unitaire d' une entité
*/			
	public List<Certificats> convertValueObjectToItems(Object ovo) throws ValidationException {
		CertificatsVO vo = (CertificatsVO) ovo;
		Certificats item = new Certificats();
		certificatsVOConverter.voToItem(vo,item);
		List<Certificats> res = new ArrayList<Certificats>();
		res.add(item);
		return res;
	}

}
