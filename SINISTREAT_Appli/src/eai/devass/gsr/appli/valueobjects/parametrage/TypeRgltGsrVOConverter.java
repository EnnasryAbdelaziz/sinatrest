...
/**
 * Value Object Converter de TypeReglement
 * 
 * @author Nom Prenom (email)
 */
public class TypeRgltGsrVOConverter implements IValueObjectConverter {
	...
	public List convertValueObjectToItems(Object ovo)
		...
		voToItem(vo, item);
		List<TypeRgltGsr> itemList = new ArrayList<TypeRgltGsr>();
		itemList.add(item);
		return itemList;
	}
	...
	public Object convertItemsToValueObject(List itemList) {
		...
		if (itemList.get(0) instanceof TypeRgltGsr) {
			itemToVo(vo, (TypeRgltGsr) itemList.get(0), itemList);
		}
	...
	public List<IValueObject> convertItemsToVos(List<IEntite> itemList) {
		...
		for (int i = 0; i < itemList.size(); i++) {
			if (itemList.get(i) instanceof TypeRgltGsr) {
				TypeRgltGsrVO vo = new TypeRgltGsrVO();
				itemToVo(vo, (TypeRgltGsr) itemList.get(i), null);
				list.add(vo);
			}
		}
		return list;
	...
	public void itemToVo(TypeRgltGsrVO vo, TypeRgltGsr item, List itemList) {
		vo.setId(item.getId());
	...
	public void voToItem(TypeRgltGsrVO vo, TypeRgltGsr item) {
		if (vo != null) {
			item.setId(vo.getId());
			if (vo.getCode() != null) {
				item.setCode(TypeConverter.getInstance().stringToDoubleObject(
						vo.getCode()));
			}
