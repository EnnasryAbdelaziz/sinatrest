...
public class InitRechercheCourrierHybridesUC extends BaseUC  {
	...
	protected void executerUC(IValueObject entite, HashMap params) throws Exception {
        ...
        logger.info("Debut UC InitRechercheCourrierHybridesUC");
		try {			
			...
			if ("1".equals(pagerVO.getNbrLignes())) {
				Integer nbreLignes = parametrageManager.getNombreCourrierHybrides(courrierHybride);
			...
			courrierList = (List<CourrierHybride>) parametrageManager.rechercheCourrierHybrides(courrierHybride,
					pagerVO);
			if (courrierList.size() == 0) {
				throw new FonctionnelleException(EXPF_AUCUN_COURRIER_TROUVE);
			}
			}else {
				courrierList = (List<CourrierHybride>) parametrageManager.rechercheCourrierHybrides(courrierHybride, null);
				if (courrierList.size() == 0) {
					throw new FonctionnelleException(EXPF_AUCUN_COURRIER_TROUVE);
				}
			}
			...
			logger.info("Fin UC InitRechercheCourrierHybridesUC");
	}
