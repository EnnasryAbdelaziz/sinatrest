...
public class RecherchePreQuittanceARisqueUC extends BaseUC {
	...
	protected void executerUC(IValueObject entite, HashMap params) throws Exception {
		...
		logger.info("Debut UC RecherchePreQuittanceARisqueUC");
		try {
			...
			if (pagerVO != null) {
				...
				if (pagerVO.getNbrLignes() != null && "0".equals(pagerVO.getNbrLignes())) {
					Integer nbreLignes = reglementManager.getNombrePreQuittanceARisque(reglement, mapDates, editer);
				...
				Iterator itr = ((List) reglementManager.getListPreQuittanceARisque(reglement, mapDates, pagerVO,
						editer)).iterator();
				while (itr.hasNext()) {
			 ...
			 * 
			 * 
			 * if (pagerVO != null && pagerVO.getNbrLignes() != null) { if
			 * (pagerVO.getNumPage() == null) { throw new FonctionnelleException(
			 * EXP_PAGER_NUMERO_PAGE_OBLIGATOIRE); } else if (pagerVO.getPageSize() == null)
			 * { throw new FonctionnelleException( EXP_PAGER_PAGE_SIZE_OBLIGATOIRE); } if
			 * ("1".equals(pagerVO.getNbrLignes())) { Integer nbreObject =
			 * reglementManager.getNombrePreQuittanceARisque( reglement, mapDates, editer);
			 * pagerVO.setNbrLignes(nbreObject.toString()); Integer pageSize =
			 * Integer.valueOf(pagerVO.getPageSize());
			 * if(nbreObject.compareTo(pageSize)>=0){ if ((nbreObject % pageSize) == 0) {
			 * pagerVO.setNbrPages(String.valueOf(nbreObject / pageSize)); } else {
			 * pagerVO.setNbrPages(String .valueOf((nbreObject / pageSize) + 1));
			 * 
			 * } } else { pagerVO.setNbrPages("1"); } }
			 * 
			 * }
			 * 
			 * List<Reglement> listQuittance = new ArrayList<Reglement>();
			 ...
			 * mapDates, pagerVO,editer)).iterator(); while(itr.hasNext()){ Object[] obj =
			 * (Object[]) itr.next(); Reglement reg = (Reglement) obj[0];
			 * listQuittance.add(reg); }
			 * 
			 * for (Reglement reg : listQuittance) { if (reg.getRefSinistre() != null) {
			...
			logger.info("Fin UC RecherchePreQuittanceARisqueUC");
