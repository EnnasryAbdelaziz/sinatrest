...
public class ValiderReglementSansOrdoUC extends BaseUC {
	...
	protected void executerUC(IValueObject entite, HashMap params)
		...
		try {
			for(Reglement reglement : listRgl) {
				...
				try {
					...
					reglementRG.setEtatReglementDB();
                        ...
                        // Set information to reglement (preparation emission
                        // QTC)
					...
					session.flush();
                        ...
                        listErrors
                                .add("Probléme lors de la mise à jour du Sinistre");
					session.clear();
					continue;
				}
				...
				try {
					reglementRG.emissionPositionnementSansOrd();
				
				} catch (IllegalAccessException e) {
                        // Dans le cas ou l'exp est au niveau du Positionnement,
                        // en continue sans rollback
                        // Il faut mettre à jour l'etat du reglement (en
				...
				try {
					reglementRG.updateCumulRegValidation();
                        ...
                        reglement.setRefEtatReglement(reglementRG
                                .getReglementDB().getRefEtatReglement());
                        reglementRG
                                .addHistoriqueEtat(IConstantes.MOTIF_VALIDATION_REGLEMENT);
				
				} catch(Exception e) {
                        // Cas exeptionnel, le positionnement et l'emission OK ,
                        // ms la mise à jour KO !!!
					// 
				}
				
				// OK
                    reglementRG.getReglementDB().setNomUserModificateur(
                            reglement.getNomUserModificateur());
                    reglementRG
                            .getReglementDB()
                            .getRefSinistre()
                            .setUserModificateur(
			...
			logger.fatal("Erreur GLOBAL lors du validation", e);
                reglementRG.getListErrors().add(
                        "Erreur GLOBAL lors du validation");
